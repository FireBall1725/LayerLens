...
final class ActiveConnection: Identifiable {
  ...
  func startNotifyStream(
    ...
    notifyTask = Task { [weak self, client, keyboard] in
      ...
      if let event = probe {
        ...
        Log.info(
          "[layerlens_notify] \(keyboard.displayName): no poll response. Push-only mode (or module not installed)")
      }
  ...
  private static func runPollLoop(
    ...
    while !Task.isCancelled {
      ...
      if now - heartbeatAt >= .seconds(5) {
        Log.debug(
          String(
            format: "[poll heartbeat] polls=%llu skipped=%llu errors=%llu lastState=0x%08X currentState=0x%08X",
  ...
  private func handleEvent(
    ...
    Log.info(
      String(
        format: "[layer] %@: state=0x%08X highest=%d",
  ...
  enum BootstrapError: Error, CustomStringConvertible {
    ...
    var description: String {
      ...
      case .unsupportedProtocolVersion(let v):
        return
          "VIA protocol v\(v) is older than v\(VIAKeycodeMap.minSupportedProtocolVersion). Update your firmware to a recent QMK build."
      }
    }
  }
}
