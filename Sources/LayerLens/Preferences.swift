...
public enum FeatureFlag: String, CaseIterable, Identifiable, Sendable {
  ...
  public var summary: String {
    ...
    case .keycodeChanger:
      return
        "Click any key in the Configure window's Keymap tab to remap it. Writes through to the keyboard's firmware over VIA."
    }
  }
...
final class Preferences {
  ...
  func setAutoConnect(_ on: Bool, for keyboard: DiscoveredKeyboard) {
    ...
    if on { autoConnectVIDPIDs.insert(key) } else { autoConnectVIDPIDs.remove(key) }
  }
