...
struct FirmwareHelpView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 18) {
      ...
      Text(
        "The floating overlay only flashes when your keyboard runs the `layerlens_notify` QMK module. The module exposes the active-layer bitmask to LayerLens over Raw HID so the overlay can mirror what's happening on the board in real time."
      )
      .font(.callout)
      .fixedSize(horizontal: false, vertical: true)

      Text(
        "The keymap viewer, custom labels, and lighting controls all work without it. Only the live layer overlay needs the module."
      )
      .font(.callout)
