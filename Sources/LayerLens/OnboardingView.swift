...
struct OnboardingView: View {
  ...
  private var welcomeStep: some View {
    VStack(spacing: 18) {
      ...
      Text(
        "A floating overlay that mirrors your QMK or Vial keyboard's active layer in real time. Live layer events, custom labels, lighting controls, and a neutral keymap viewer for any board on the VIA registry."
      )
      .font(.callout)
  ...
  private var connectStep: some View {
    VStack(spacing: 14) {
      ...
      Text(
        "LayerLens auto-detects QMK and Vial keyboards over USB. If yours isn't listed, double-check that VIA support is enabled in its firmware."
      )
      .font(.callout)
      .foregroundStyle(.secondary)
      .multilineTextAlignment(.center)
  ...
  private var permissionsStep: some View {
    VStack(spacing: 14) {
      ...
      Text(
        "LayerLens can highlight each key as you press it on the floating overlay. macOS asks for permission to read keyboard events the first time, so other apps' keystrokes can light up the right key."
      )
      .font(.callout)
      .foregroundStyle(.secondary)
      .multilineTextAlignment(.center)
  ...
  private var startAtLoginStep: some View {
    ...
    return VStack(spacing: 14) {
      ...
      Text(
        "LayerLens lives in your menu bar. Most users want it running whenever they're at their Mac. Turn this on and it'll start automatically each time you log in."
      )
      .font(.callout)
      .foregroundStyle(.secondary)
      .multilineTextAlignment(.center)
  ...
  private var privacyStep: some View {
    VStack(spacing: 14) {
      ...
      Text(
        "LayerLens can send a small set of anonymous usage signals so we know which keyboards and macOS versions to prioritise. It's off by default and you can flip it any time in Settings → Privacy."
      )
      .font(.callout)
      .foregroundStyle(.secondary)
      .multilineTextAlignment(.center)
      ...
      VStack(alignment: .leading, spacing: 8) {
        privacyRow(
          ...
          title: "Sent",
          body:
            "macOS version, CPU type, app version, locale, keyboard VID:PID, whether the firmware module is installed."
        )
        privacyRow(
          ...
          title: "Never sent",
          body:
            "Keystrokes, layer contents, custom labels, your name, hostname, IP address, or anything that could identify you."
        )
      }
  ...
  private var finishStep: some View {
    VStack(spacing: 14) {
      ...
      VStack(alignment: .leading, spacing: 10) {
        ...
        bulletRow(
          "eye",
          "Pick \"Show Overlay\" to pin the floating layer view, or just press a layer key. It'll flash automatically.")
