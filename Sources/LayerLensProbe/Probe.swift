...
struct Probe {
  static func main() async {
    ...
    if keyboards.isEmpty {
      print(
        "No Raw-HID keyboards found (usage page 0x\(hex16(RawHIDConstants.viaUsagePage)), usage 0x\(hex8(RawHIDConstants.viaUsage)))."
      )
    ...
    guard let first = keyboards.first,
      let raw = HIDEnumerator.resolveDevice(for: first)
    else {
    ...
    do {
      ...
      do {
        ...
        print(
          "✓ Resolved: \(definition.rows) rows × \(definition.cols) cols, \(definition.layouts.first?.keys.count ?? 0) physical keys"
        )
      } catch let LayoutResolver.ResolveError.notInManifest(vid, pid) {
        print(
          "⚠ \(String(format: "%04X:%04X", vid, pid)) is not in the VIA keyboards repo. Falling back to matrix-only output."
        )
